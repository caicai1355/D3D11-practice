...
struct ModelColliderDataAABB:public Model
{
	ModelColliderDataAABB()
	{
		type = MODEL_NULL;
	}
...
XMVECTORF32 eyePos = {-1.0f,3.0f,-1.0f,1.0f};
	...
	if(modelFile)
	{
		...
		if(isCalcNormal)
		{
			...
			for(int i = 0,iLen = modelData->indexVec.size();i < iLen;i+=3)
			{
				...
				vectorTemp = XMVector3Cross(edge1,edge2);
				if((XMVectorGetX(vectorTemp) != 0.0f || XMVectorGetY(vectorTemp) != 0.0f || XMVectorGetZ(vectorTemp) != 0.0f))
				{
					vectorTemp = XMVector3Normalize(vectorTemp);
				}
				XMStoreFloat3(&normalTemp,vectorTemp);
			...
			for(int i = 0,iLen = modelData->vertexVec.size();i < iLen;i++)
			{
				...
				{
					if((faceNormal[j].x != 0.0f || faceNormal[j].y != 0.0f || faceNormal[j].z != 0.0f) && (modelData->indexVec[j*3] == i || modelData->indexVec[j*3+1] == i || modelData->indexVec[j*3+2] == i))
					{
						joinFaceNum++;
			...
			else if(keyString == L"mesh")
			{
				...
				if(isCalcNormal)	//calculate the normal direction
				{
					...
					for(int i = 0,iLen = meshTemp.vertexData.size();i < iLen;i++)
					{
						...
						for(int j = 0,k = 0,jLen = faceNormal.size();j < jLen;j++)
						{
							if((faceNormal[j].x != 0.0f || faceNormal[j].y != 0.0f || faceNormal[j].z != 0.0f) && (meshTemp.indexData[j*3] == i || meshTemp.indexData[j*3+1] == i || meshTemp.indexData[j*3+2] == i))
							{
								joinFaceNum++;
								normalTemp.x += faceNormal[j].x;
								normalTemp.y += faceNormal[j].y;
...
bool LoadMD5Anime(std::wstring filename,struct MD5animData *md5animData,bool isRHCoord,bool isCalcNormal = true)
{
	std::wifstream md5Anim(filename);
	if(md5Anim)
	{
		std::wstring keyString;
		std::wstring commandline;
...
bool MouseHitDetect(Model & modelData,CXMMATRIX worldSpaceTemp,int method = DETECT_METHOD_MODEL)
{
	...
	if(modelData.type == MODEL_COLLIDER_AABB)	//detect the AABB box
	{

	}
	else	//detect the bounding box
	{
		std::vector<Vertex> *vertexVec;
		std::vector<DWORD> *indexVec;
		if(modelData.type == MODEL_ISINIT)
		{
			vertexVec = &((ModelData&)modelData).vertexVec;
			indexVec = &((ModelData&)modelData).indexVec;
		}
		else
		{
		...
		{
			point1.v =  XMVector3TransformCoord(XMLoadFloat3(&((*vertexVec)[(*indexVec)[i]].position)),worldSpaceTemp);
			point2.v =  XMVector3TransformCoord(XMLoadFloat3(&((*vertexVec)[(*indexVec)[i+1]].position)),worldSpaceTemp);
			point3.v =  XMVector3TransformCoord(XMLoadFloat3(&((*vertexVec)[(*indexVec)[i+2]].position)),worldSpaceTemp);
